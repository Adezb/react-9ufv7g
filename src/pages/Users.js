...
const Users = () => {
  ...
  const pages = 15;
  const skip = page * Each_Page - Each_Page;
  ...
  return (
    <div className="page-content"> 
     ...
     onClick = {() => setPage((prev) => prev - 1)} className="nav-btn"> Prev</button>
     }
     ...
     onClick = {() => setPage((prev) => prev + 1)} className="nav-btn">Next</button>
     }
     {/*SETTING PAGE NUMBER BUTTON */}
     {Array.from({length: pages}, (value, index) => index + 1).map(
       (each) => (<button onClick = {() => setPage(each)} className="page-numb-btn">{each}</button>)
     )}
